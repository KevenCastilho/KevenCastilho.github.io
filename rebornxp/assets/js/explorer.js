...
$(window).on('xpboot', function() {
  ...
  explorer.specialLocations = {
    'Meu Computador': [['dir', 'hardDrive', 'Disco Local (C:)', '/']],
    'Painel de Controle': []
  };
  let temp = [];
...
var explorer = {
  ...
  register: function() {
    xp.applications.add('explorer', (args) => {
      ...
      args = args || [];
      var path = args[0] || '/Documents and Settings/' + xp.profile.name + '/Meus Documentos';
      ...
      <div class="explorer_files yscroll">
        ...
        <div class="hiddenfiles" style="display:none;">
          <h1>Esses arquivos estão ocultos.</h1>
          <h3>Esta pasta contém arquivos que mantêm seu computador funcionando corretamente.<br/>
          You should not modify its contents.</h3>
          <a href="" onclick="$(this).closest('.hiddenfiles').css('display', 'none');return false;">Mostrar o conteúdo desta pasta</a>
        </div>
      </div>
      ...
      $('#openfilemenu_' + guid).closest('.explorer_files').on('contextmenu', function(e) {
        e.stopPropagation();
        e.preventDefault();
        vContextMenu = Array('Novo Arquivo', 'explorer_action(\'newfile\', \'' + guid + '\');', 'Nova Pasta', 'explorer_action(\'newdir\', \'' + guid + '\');');
...
function explorer_action(type, guid) {
  ...
  if (type === 'newfile') {
    xp.prompt('Novo Arqivo', 'Digite o nome do Arquivo:', function(text) {
    ...
    xp.prompt('Nova Pasta', 'Digite o nome da nova Pasta:', function(text) {
...
function exp_ofp(path, guid, winguid, callback, saveFile) {
  ...
  $('#moveUp_' + guid).off().on('click', () => {
    ...
    if (path2 === "") path2 = "/";
    if (path2.charAt(0) !== '/') path2 = '/' + path2;
	  if (path === '/' || path === 'Meu Computador') path2 = 'Meu Computador';
  ...
  if (explorer.specialLocations[path] !== undefined) {
        ...
        $(el).on('click', function(event) {
          ...
          if ($(this).data('selected') !== 'true') {
            ...
            if (loc[i][4] === undefined)
              xp.dialog('Error', loc[i][2] + ' é um arquivo protegido do sistema.<br/>Você não tem permissão para abrir.', () => {}, false, 'error');
            else
              loc[i][4]();
            $('._file_' + guid).each(function() {
              $(this).data('selected', 'false');
    ...
    xp.filesystem.listDir(path, function(e) {
      if (typeof e !== 'string') {
        xp.dialog('Error', '' + e + '<br/>Pasta Solicitada: "' + path + '"', () => {}, false, 'error');
...
function explorer_delete(el2, guid) {
  ...
  var el = $(el2).closest('window');
  if ($(el2).attr('filename') !== undefined) {
    xp.dialog('Delete File', 'Tem certeza de que deseja excluir ' + $(el2).attr('filename') + '?', function() {
    ...
    xp.dialog('Delete Directory', 'Tem certeza de que deseja excluir ' + $(el2).attr('dirname') + '?', function() {
      xp.audio.playURL('https://cdn.glitch.com/01d2e04f-e49d-4304-aa9e-55b9849b4cce%2FWindows%20XP%20Recycle.wav?1522621854189');
...
xp.filesystem.openFileDialog = function(callback) {
  var guid = generate_guid();
  var path = '/Documents and Settings/' + xp.profile.name + '/Meus Documentos';
  var el = $.parseHTML(`<window title="Open File" width="480" height="360">
      ...
      <div class="explorer_files yscroll" style="height:calc(100% - 77px);">
        ...
        <div class="hiddenfiles" style="display:none;height:calc(100% - 97px);">
          <h1>Esses arquivos estão ocultos.</h1>
          <h3>Esta pasta contém arquivos que mantêm seu computador funcionando corretamente.<br/>
          Você não deve modificar seu conteúdo.</h3>
          <a href="" onclick="$(this).closest('.hiddenfiles').css('display', 'none');return false;">Mostrar o conteúdo desta pasta</a>
        </div>
      </div>
      <footer>
