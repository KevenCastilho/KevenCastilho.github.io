...
<div style="margin-left:32px;">
    <h1 style="font-weight:normal;">Obrigado por escolher o RebornXP!</h1>
    <h2 style="font-weight:normal;">Escolha um nome de usuario e uma imagem</h2>
    <p>
        Seu nome de usuario e sua imagem representão sua conta. A conta que você está criando aqui é <br>
    ...
    <p style="margin-left:64px;">
        <table style="font-size:11px;color:#000;">
            ...
            <tr><td>Digite um nome (Por Exemplo, Keven)</td></tr>
    ...
    <div style="position:absolute;right:8px;bottom:8px;">
        <button disabled class="next">Proximo</button>
    </div>
</div>
`);
...
win.el.find('.next').on('click', function() {
    if (win.el.find('.username').val() === '') {
        xp.alert('Você precisa escrever um nome para continuar!');
    } else {
        ...
        xp.wallpaper.href = 'assets/imagens/Desktop.jpg';
        win.content(`
<div style="margin-left:32px;">
    <h1 style="font-weight:normal;">Por favor aguarde enquanto configuramos seu desktop.</h1>
    <h2 style="font-weight:normal;">Seu computador reiniciará diversas vezes durante o processo.</h2>
</div>`);
        setTimeout(() => {
            requiredDirectories = [
                `/Documents and Settings`,
                `/Documents and Settings/${xp.profile.name}`,
                `/Documents and Settings/${xp.profile.name}/Meus Documentos`,
				`/Documents and Settings/${xp.profile.name}/Meus Documentos/Minhas Imagens`,
				`/Documents and Settings/${xp.profile.name}/Meus Documentos/Meus Videos`,
				`/Documents and Settings/${xp.profile.name}/Meus Documentos/Minhas Musicas`
            ];
            var i = 0;
            function createDirs() {
